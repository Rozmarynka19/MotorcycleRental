...
class Firma final :private Klient //jak wyzej
{
	...
	friend void wyswietl_baze_firm(vector <Firma>&);
	friend void usun_ostatnio_dodana_firme(vector <Firma>&, vector <Motocykl>&);
	...
	friend void wypozycz_motocykl_firmie(vector <Motocykl>&, vector <Firma>&);
public:
...
void wyswietl_baze_firm(vector <Firma>& baza_klientow_firma)
{
	//system("cls");
...
void usun_ostatnio_dodana_firme(vector <Firma>& baza_klientow_firma, vector <Motocykl>& baza_motocykli)
{
	int iterator = baza_klientow_firma.size() - 1;
	cout << "Zostanie usunieta firma o nastepujacych danych:\n";
	baza_klientow_firma[iterator].pokaz_dane();
	cout << "\nCzy kontynuowac? (0-nie,1-tak): ";
	cin >> odp;
...
void wypozycz_motocykl_firmie(vector <Motocykl>& baza_motocykli, vector <Firma>& baza_klientow_firma)
{
			...
			else if (odp == 2)//baza firm
			{
				do
				{
					...
					case 2: //dodaj firme
					{
						...
						baza_klientow_firma.push_back(Firma(nazwa, regon));
